...
void ShutTheBox::csv_record_unreachable_positions(std::ostream &csv_out, unordered_set<string> &reached_positions) {
...
unordered_set<uint32_t> ShutTheBox::get_combination(const uint32_t roll_num) {
    ...
    set_to_combination(roll_num, tile_combination);
    return tile_combination;
}
void ShutTheBox::set_to_combination(const uint32_t roll_num, unordered_set<uint32_t> &tile_combination) {
...
Results ShutTheBox::probability_of_strategy_victory_step(uint32_t score_in) {
    ...
    if (num_face_up == 1) {
        if(score_in == 1) {
            results.win_probability = dice.get_probability(score_in, 1);
            results.avg_score = (1 - dice.get_probability(score_in, 1)) * score_in;
        } else {
