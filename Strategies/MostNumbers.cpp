...
MostNumbers::MostNumbers(unordered_set<uint32_t> tiles_in, unordered_map<uint32_t, unordered_map<uint32_t, double>> dice_probabilities_in): 
    ShutTheBox(tiles_in, dice_probabilities_in, "Most Numbers", "../results/most_numbers.csv") { 
}

void MostNumbers::set_combination(const uint32_t roll_num, unordered_set<uint32_t>& tile_combination) {
